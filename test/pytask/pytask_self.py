...
def pytask_self_exec_proxy_test(vamos_task):
    def task(ctx, task):
        exec_proxy = ctx.proxy_mgr.get_exec_lib_proxy()
...
def pytask_self_dos_proxy_test(vamos_task):
    def proc(ctx, task):
        dos_proxy = ctx.proxy_mgr.get_dos_lib_proxy()
        # IoErr
        err = dos_proxy.IoErr()
        assert err == 0
        return 42
...
def pytask_self_vlib_proxy_test(vamos_task):
    def task(ctx, task):
        ...
        vlib = ctx.proxy_mgr.open_lib_proxy("vamostest.library")
        assert vlib is not None
        # call lib func
        result = vlib.Add(21, 42)
        assert result == 63
        # close lib
        ctx.proxy_mgr.close_lib_proxy(vlib)
        return 42
