...
class ADFSBitmap:
  ...
  def draw_on_bitmap(self, bm):
    ...
    bm[0:res] = b"x" * res
    # root block
    bm[self.root_blk.blk_num] = b'R'
    # bitmap blocks
    for bm_blk in self.bitmap_blks:
      bm[bm_blk.blk_num] = b'b'
    # bitmap ext blocks
    for ext_blk in self.ext_blks:
      bm[ext_blk.blk_num] = b'B'
  ...
  def print_draw_bitmap(self, bm, brief=False):
    ...
    for i in range(self.blkdev.num_blocks):
      c = bm[i]
      if c == 0 or (not isinstance(c, int) and ord(c) == 0):
        c = '.'
      else:
