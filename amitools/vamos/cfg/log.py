...
class LogParser(Parser):
  def __init__(self, ini_prefix=None):
    ...
    arg_cfg = {
        "logging": {
            "levels": Argument("-l", "--logging", action='store',
                               help="logging settings: <chan>:<level>,*:<level>,..."),
            "file": Argument("-L", "--log-file", action='store',
                             help="write all log messages to a file"),
            "verbose": Argument("-v", "--verbose", action='store_true',
                                help="be more verbos"),
            "quiet": Argument("-q", "--quiet", action='store_true',
                              help="do not output any logging"),
            "timestamps": Argument('--no-ts', action='store_false',
                                   help="do not log with timestamp")
        }
    }
    ini_trafo = {
      "logging": {
