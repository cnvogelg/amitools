...
from amitools.vamos.astructs import AccessStruct, MemHeaderDef, MemChunkDef
...
class MemChunk:
  ...
  def __str__(self):
    return "[MC(%06x+%06x=%06x):next=%06x]" % \
      (self.addr, self.bytes, self.addr+self.bytes, self.next)
...
class MemHdr:
  ...
  def __str__(self):
    return "[MH@%06x:(%06x+%06x=%06x):free=%06x,MC=%06x]" % \
      (self.addr, self.lower, self.total, self.upper, self.free, self.first)
