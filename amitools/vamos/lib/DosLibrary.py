...
class DosLibrary(LibImpl):
  ...
  def Lock(self, ctx):
    ...
    lock = self.lock_mgr.create_lock(self.get_current_dir(ctx), name, lock_exclusive)
    log_dos.info("Lock: (%s) '%s' exc=%s -> %s" % (self.get_current_dir(ctx), name, lock_exclusive, lock))
    if lock == None:
      self.setioerr(ctx,ERROR_OBJECT_NOT_FOUND)
      return 0
    else:
  ...
  def SystemTagList(self, ctx):
    ...
    if ctx.process.is_native_shell():
      ...
      ret_code = ctx.vamos.run_command(ctx.scheduler, ctx.process, code_start, 0, 0, stack_size, reg_d1)
  ...
  def RunCommand(self, ctx):
    ...
    ret_code = ctx.vamos.run_command(ctx.scheduler, ctx.process, prog_start, args, length, stack)
    # clear input
    input_fh.setbuf("")
    return ret_code
