...
class MsgPort(MsgPortStruct):
    def new(self, pri=0, flags=0, sig_bit=0, sig_task=0, nt=NodeType.NT_MSGPORT):
        self.pri.val = pri
        self.type.val = nt
        self.flags.val = flags
        self.sig_bit.val = sig_bit
        self.sig_task.ref = sig_task
        self.msg_list.new(NodeType.NT_MESSAGE)


@AmigaClassDef
class Message(MessageStruct):
    def new(self, pri=0, reply_port=0, length=0, nt=NodeType.NT_MESSAGE):
        self.pri.val = pri
        self.type.val = nt
