...
class AccessStruct(AccessMemory):
  def __init__(self, mem, label_mgr, struct_def, struct_addr=0):
    AccessMemory.__init__(self,mem, label_mgr)
    if struct_addr == 0:
  ...
  def r_s(self, name):
    ...
    if conv != None:
      val = conv[1](val)
    self._trace_read(width, addr, val, text="Struct", addon="%s+%d = %s" % (self.struct_type_name, off, name), level=logging.INFO)
    return val
