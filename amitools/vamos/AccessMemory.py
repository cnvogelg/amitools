...
class AccessMemory():
  ...
  def _get_mem_str(self, addr):
    return "@%06x +%06x %s" % (self.mem.addr, addr - self.mem.addr, self.mem.name)
  ...
  def w_data(self, addr, data):
    off = addr
    for d in data:
      self.mem.write_mem_int(0, off, ord(d))
      off += 1
  ...
  def r_data(self, addr, size):
    ...
    for i in xrange(size):
      result += chr(self.mem.read_mem_int(0, off))
      off += 1
  ...
  def r_cstr(self, addr):
    ...
    v = None
    while True:
      v = chr(self.mem.read_mem_int(0, off))
      if v == '\0':
        break
      res += v
      off += 1
  ...
  def w_cstr(self, addr, cstr):
    off = addr
    for c in cstr:
      self.mem.write_mem_int(0, off, ord(c))
      off += 1
    self.mem.write_mem_int(0, off, 0)
    self._trace_block_write( addr, len(cstr), text="CSTR", addon="'%s'"%cstr, level=logging.INFO )

  def r_bstr(self, addr):
    off = addr
    res = ""
    size = self.mem.read_mem_int(0, off)
    off += 1
    for i in xrange(size):
      res += chr(self.mem.read_mem_int(0, off))
      off += 1
  ...
  def w_bstr(self, addr, bstr):
    off = addr
    size = len(bstr)
    self.mem.write_mem_int(0, off, size)
    off += 1
    for c in bstr:
      self.mem.write_mem_int(0, off, ord(c))
      off += 1
